...
/// Registers the status subsystem's two channels into the registry: the
/// consumer-side push channel every `/ws` session subscribes through, and
/// the producer-side sink same-tier subsystems emit through. The returned
